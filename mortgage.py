...
def get_min_payment(principal, annual_interest_rate, mortgage_term = 30, num_payments = 12):

    """Computes the minimum mortgage payment"""

    p = principal
    r = annual_interest_rate / num_payments
...
def interest_due(mortgage_balance, annual_interest_rate, num_payments = 12):
...
def remaining_payments(mortgage_balance, annual_interest_rate, target_payment, num_payments = 12):
    
    counter = 0
    while mortgage_balance > 0:
        interest = interest_due(mortgage_balance, annual_interest_rate, num_payments = 12)
        principal_balance = target_payment - interest
        mortgage_balance -= principal_balance
        counter += 1
    return counter

def main(principal, annual_interest_rate, mortgage_term = 30, num_payments = 12, target_payment = None,):
    
    min_payment = get_min_payment(principal, annual_interest_rate, mortgage_term = 30, num_payments = 12)
    ...
    if target_payment < min_payment:
        ...
        mortgage_amount = remaining_payments(principal, annual_interest_rate, target_payment, num_payments = 1)
        print(f"If you make payments of ${target_payment}, you will pay off the mortgage in ${mortgage_amount} payments")
